...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isCodeValid) {
      return serveApiResponse(false, "Verification code is incorrect. Please sign up again to receive a new code.", 400);
    }

    const isCodeNotExpired = new Date(user.verifyCodeExpiry) > new Date();
    if (!isCodeNotExpired) {
      return serveApiResponse(false, "Verification code has expired. Please sign up again to receive a new code.", 400);
    }
    ...
    return serveApiResponse(false, "An unexpected error occurred during verification. Please try again.", 500);
  }
}
